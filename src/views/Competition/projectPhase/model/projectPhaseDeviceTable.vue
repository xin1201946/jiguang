  ...
  <div>
    <div v-if="cproId">
      <div style="margin-bottom: 20px" v-if="!disabled">
        <a-space>
          <a-button type="primary" @click="handleAdd" icon="plus">添加
          </a-button>
        </a-space>
      </div>
      <a-table bordered :pagination="pagination" :data-source="data" @change="handleTableChange" rowKey="cproDeviceId" size="small" :columns="columns" :scroll="{ y: 200, x: 1000}">
        <template slot="targetSite" slot-scope="text, record, index">
          ...
          <a-input v-else :disabled="disabled" oninput="value=value.replace(/[^\d]/g,'')" @focus="handleBindFocus(record, index)" v-model="record.targetSite" @blur="handleBind(record)"></a-input>
        </template>
