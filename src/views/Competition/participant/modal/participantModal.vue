      ...
      <a-form-model :labelCol="{span: 8}" :wrapperCol="{span: 14}" :rules="rules" ref="form" :model="formData">
        ...
        <a-form-model-item label="项目一(10米手枪)">
          <a-select allowClear v-model="formData.project1">
            <a-select-option v-for="item in list.filter(i => i.projectGroup === formData.projectGroup).filter(i => i.projectName.includes('手枪'))" :key="item.cproId" :value="item.projectName">{{ item.projectName }}
            </a-select-option>
          </a-select>
        </a-form-model-item>
        ...
        <a-form-model-item label="项目二(10米步枪)">
          <a-select allowClear v-model="formData.project2">
            <a-select-option v-for="item in list.filter(i => i.projectGroup === formData.projectGroup).filter(i => i.projectName.includes('步枪'))" :key="item.cproId" :value="item.projectName">{{ item.projectName }}
            </a-select-option>
          </a-select>
        </a-form-model-item>
        ...
        <a-form-model-item label="项目三(混合团体项目)">
          <a-select allowClear v-model="formData.project3">
            <a-select-option v-for="item in list.filter(i => i.projectName.includes('团体'))" :key="item.cproId" :value="item.projectName">{{ item.projectName }}
            </a-select-option>
          </a-select>
        </a-form-model-item>
