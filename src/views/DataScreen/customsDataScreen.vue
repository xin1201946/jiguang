<template>
  <div style="width: 100%; display: flex" v-show='!len'>
    <a-carousel style="width: 100%; height: 100%" ref="carousel">
  ...
  methods: {
      ...
      littleScreen({
        ...
        if (result) {
          if (result.players && Array.isArray(result.players)) {
            ...
            console.log('处理后的自定义屏幕决赛选手数据:', data.finalEight)
          } else {
            console.warn('自定义屏幕决赛数据中没有players数组或为空')
            data.finalEight = []
          }
        }
    ...
    tuanti(data) {
      getTeamScoresAPI({
        ...
        data.th.push({ name: '个人成绩', width: '80px' }, { name: '总环数', width: '100px' })
